...
# Import plugins với xử lý lỗi
try:
    from folium.plugins import SimpleScreenshot, MousePosition, MeasureControl
except ImportError:
    st.error("Thiếu thư viện plugins. Hãy đảm bảo đã thêm 'folium' vào requirements.txt")
...
st.set_page_config(page_title="Hệ thống Theo dõi Bão - Modular", layout="wide")

# CSS: GIỮ NGUYÊN CODE NỀN (Full màn hình, không khoảng trắng)
st.markdown("""
    <style>
...
if os.path.exists(DATA_FOLDER):
    ...
    sel_curr = st.sidebar.multiselect("Bão hiện tại:", options=all_files, default=all_files[:1] if all_files else [])
    ...
    for f in all_files:
        try:
            full_dfs.append(pd.read_excel(os.path.join(DATA_FOLDER, f), sheet_name='besttrack'))
        except: pass
    
    if full_dfs:
        combined = pd.concat(full_dfs).dropna(subset=['lat', 'lon'])
        combined[['lat', 'lon']] = combined[['lat', 'lon']].apply(pd.to_numeric)
        
        sel_nums = st.sidebar.multiselect("Lọc bão:", options=sorted(combined['Số hiệu'].unique().tolist()))
        sel_bf = st.sidebar.slider("Cấp gió:", 0, 18, (0, 18))

        # Nhúng Option 1 (Hiện tại - Màu Đỏ)
        ...
        # Nhúng Option 2 (Quá khứ - Màu Xanh)
        if sel_nums:
            df_filt = combined[(combined['Số hiệu'].isin(sel_nums)) & (combined['cường độ (cấp BF)'].between(sel_bf[0], sel_bf[1]))]
