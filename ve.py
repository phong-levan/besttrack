...
# BẮT BUỘC DÙNG FILE SHAPEFILE GỐC CỦA NGƯỜI DÙNG (vn34tinh)
# ==============================================================================
def run_interactive_folium_interpolation(input_df, title_text, cmap_name, num_bins, custom_levels, selected_provinces, shape_col, custom_bounds=None):
    ...
    # 2. Đọc ranh giới hành chính BẮT BUỘC từ vn34tinh.shp (để chuẩn VN)
    if not os.path.exists(SHP_MASK_PATH):
        return None, None, f"Không tìm thấy file ranh giới {SHP_MASK_PATH}."
    ...
    if selected_provinces:
        if shape_col in mask_shape.columns:
            ...
            if found_col:
                ...
                return None, None, "Lỗi: Không tìm thấy tỉnh đã chọn trong file Shapefile."
    else:
    ...
    # 4. Cắt dữ liệu (Clip) theo đúng ranh giới Việt Nam (hoặc tỉnh)
    ...
    # Lớp ảnh nội suy (Opacity 0.85 cực kỳ rõ nét, bám khít biên giới)
    ...
    # ==============================================================================
    # 7. TẠO FIGURE TĨNH CHO FILE TẢI (Sửa lỗi to chữ, thêm nền, tăng đậm)
    # ==============================================================================
        ...
        if topic == "Dữ liệu quan trắc":
            if st.session_state['logged_in']:
                ...
                # --- MENU NỘI SUY TƯƠNG TÁC (FOLIUM) ---
                    ...
                    # Quét file SHP CHÍNH (vn34tinh.shp)
                    province_list = []
                    shape_col = "NAME_1"
                    if os.path.exists(SHP_MASK_PATH):
                        try:
