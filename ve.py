...
# CSS: SỬA LỖI CUỘN SIDEBAR VÀ GIỮ NỀN TRÀN VIỀN
st.markdown("""
    <style>
    /* Cho phép cuộn ở Sidebar nhưng khóa cuộn ở trang chính */
    ...
    /* Xóa khoảng cách nội dung chính */
...
def create_base_map():
    ...
    for lon in range(0, 360, 5):
        folium.PolyLine([[-90, lon], [90, lon]], color='black', weight=1.0, opacity=1.0).add_to(m)
    for lat in range(-90, 90, 5):
        folium.PolyLine([[lat, 0], [lat, 360]], color='black', weight=1.0, opacity=1.0).add_to(m)
...
def draw_storm_data(fg, df_raw, color="black", show_swaths=False):
    if show_swaths:
        ...
        if polys_r6 or polys_r10 or polys_rc:
...
if os.path.exists(DATA_FOLDER):
    ...
    if all_dfs:
        ...
        storm_list = sorted(combined['Số hiệu'].unique().tolist())
        sel_nums = st.sidebar.multiselect("Lọc bão quá khứ:", options=storm_list)
